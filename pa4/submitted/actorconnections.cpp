...
void findConnections(ActorGraph*, queue< pair<string, string> >& target,
                      ofstream&, bool);
void findConnectionsUfind(UnionGraph*, queue< pair<string, string> >&, ofstream&);
...
int main(int argc, char* argv[]){
  ...
  // bfs case
  if(bfs){
    auto graph = new ActorGraph(weighted);
  ...
  // ufind case
    ...
    cout << "#movies: " << uGraph->getAllMovies().size() << endl;
    cout << "done\n";
    ...
    Timer timer;
    timer.begin_timer();
    findConnectionsUfind(uGraph, actorPairs, outFile);
    cout << "\nThe duration in milliseconds is : " << timer.end_timer() << endl;
...
void findConnectionsUfind(UnionGraph* uGraph, queue< pair<string, string> >& target, ofstream& out){

  while(target.size()) {

    int connectYear  = uGraph->uFind(target.front().first, target.front().second);

    out << target.front().first << "\t" << target.front().second << "\t" 
        << connectYear << "\n";
