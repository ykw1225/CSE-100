...
void findPath(ActorGraph* graph, queue< pair<string, string> >& target, ofstream& out){
  ...
  // the start actor's name in last iteration
  string prev = "";

  // boolean for indicate first iteration
  bool first = true;
...
//cout << "size of queue:" << target.size() << endl;
    ...
    // not first iteration
      ...
      // start actor's name is not same as last iteration
      if(target.front().first.compare(prev))
        find = graph->findPath(target.front().first, target.front().second);
...
//else cout << "\tFindpath fail!\n";
    ...
    // update previous start actor's name
