...
void HCTree::build(const vector<int>& freqs){
  ...
  for( int i = 0; i < freqs.size(); i++ ){
    if( freqs[i] ) {
      // make a HC node for the newly appeared symbol
...
int HCTree::decode(BitInputStream& in)const{
  ...
  while(currNode->c0 || currNode->c1){
    ...
    // if this bit is neither 1 nor 0
      ...
      cout << "\n\tUnexpected number (" << n << ") found!" << endl;
      return -1;
    }
  } // end of while
...
void HCTree::printNode(HCNode* node){
  if(!node) return;
  cout << "\nSymbol: " << (int)(node->symbol) << " Count: " << node->count;
  if(node->c0)
    cout << " Child0: " << (int)(node->c0->symbol);
  if(node->c1)
    cout << " Child1: " << (int)(node->c1->symbol);
  if(node->p)
    cout << " Parent: " << (int)(node->p->symbol);
  if(node->c0)
    printNode(node->c0);
  if(node->c1)
    printNode(node->c1);
...
void HCTree::printAll(){
  ...
  for(int i = 0; i < leaves.size(); i++){
    if(leaves[i]){
      cout << "Leave " << i << ": ";
      cout << "Symbol: " << (int)(leaves[i]->symbol)
