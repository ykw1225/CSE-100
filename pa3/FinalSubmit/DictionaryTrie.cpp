 ...
 * Description: This is the cpp file for DictionaryTrie class
 *              it has functions to build the trie, check if a
 *              word is in the dictionary, find highest frequency
 *              words for autocompletions.
 */
...
bool DictionaryTrie::insert(std::string word, unsigned int freq) {
  ...
  // check empty string and invalid frequency
  if(!word.length()) return false;

  // temporary pointer to track current node
  auto curr = createPath(word, freq);

  // if fail to create the path for word
...
/** Find if the given word is stored in the dictionary
 *  Return true if the word was found, and false if it
 *  was not
 */
bool DictionaryTrie::find(std::string word) const{
  ...
  // temporary variable
  auto n = findPath(word);

  // if the node is not exist
  if(!n || !n->word.length()) return false;
...
std::vector<std::string> DictionaryTrie::predictCompletions(std::string prefix,
                                            unsigned int num_completions)const{
  // return value
  vector<string> word;
  ...
  // transfer num_completions number of words into vector from set
  while(s.size()){
      
    // iterator pointer to past the largest frequency word in set
    set<Node>::iterator itr = s.end();
    
    // decrement iterator to point the largest frequency word in set
    itr--;
    
    // push the largest frequency word in set
    word.push_back((*itr).word);
    
    // delete the largest frequency word in set
    s.erase(itr);
  }
...
/** creates a trie node, set all child nodes to 0
 *  return a pointer to that node
 */
Node* DictionaryTrie::addNode(string word, unsigned int freq){
  
  // create a node object in heap
  Node* x = new Node;
  
  // assign the string word
  x->word = word;
  
  // aasign the frequency of word
  x->freq = x->max = freq;
  
  // initialize children node to 0
  x->a = x->b = x->c = x->d = x->e = x->f = x->g = x->h = x->i =
  x->j = x->k = x->l = x->m = x->n = x->o = x->p = x->q = x->r = 
  x->s = x->t = x->u = x->v = x->w = x->x = x->y = x->z = x->space = 0;
 ...
 */
Node* DictionaryTrie::createPath(string word, unsigned int freq){
  ...
  for(unsigned int i=0; i < word.length(); i++){
    ...
    // check current letter of word to indicate which path should choose
    switch(word[i]){
      
      case 'a': // check if the child node exist, if not then create a new node
                if(!curr->a) curr->a = addNode(freq);
      
                // if child node exist, compare the max member and freq, then update max
                else if(curr->a->max < freq) curr->a->max = freq;
                  
                // update current pointer to the childe node
                curr = curr->a;
...
Node* DictionaryTrie::findPath(string word)const{
  ...
  for(unsigned int i=0; i < word.length(); i++){
    ...
    switch(word[i]){
      ...
      case 'a': // if child node not exist, return null
                if(!curr->a) return nullptr;
                
                // if child node exist, update current node to child node
                curr = curr->a;
