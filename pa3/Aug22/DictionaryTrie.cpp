...
bool DictionaryTrie::insert(std::string word, unsigned int freq) {
  ...
  if(!curr->word.length()){
    curr->word = word;
    curr->freq = freq;
    // update the max freq in the subtree rooted by the new node
    //if(curr->max < freq) curr->max = freq;
    return true;
  }
...
std::vector<std::string> DictionaryTrie::predictCompletions(std::string prefix,
  ...
  // store num_completions number of words into set
...
void DictionaryTrie::printAll()const{
  if(!root)
    ...
    cout << "\t" << left << setw(10) << "Words:" << setw(10) << "Freqs:" << "Max:" << endl;
    for(; itr != v.end(); itr++)
      cout << "\t" << left << setw(10) << (*itr)->word << setw(10) << (*itr)->freq << (*itr)->max << endl;
 ...
 */
unsigned int DictionaryTrie::findFreq(string word)const{
  if(!word.length()) return 0;
  Node* n = findPath(word);
  unsigned int r = 0;
  if(!n)cout << "Word is not found!!!";
  else r = n->freq;
  return r;
}

/** find all words rooted by n and store in v
 */
void DictionaryTrie::findWords(Node* n, vector<Node*>* v)const{
  if(!n || !v) return;
  if(n->word.length())
    v->push_back(n);
