...
void DictionaryTrie::findWords(Node* n, priority_queue<Node*, vector<Node*>, NodeComp> &q)const{
  if(!n) return;
  if(n->word.length()) q.push(n);
  if((n->a)) findWords(n->a, q);
  if((n->b)) findWords(n->b, q);
  if((n->c)) findWords(n->c, q);
  if((n->d)) findWords(n->d, q);
  if((n->e)) findWords(n->e, q);
  if((n->f)) findWords(n->f, q);
  if((n->g)) findWords(n->g, q);
  if((n->h)) findWords(n->h, q);
  if((n->i)) findWords(n->i, q);
  if((n->j)) findWords(n->j, q);
  if((n->k)) findWords(n->k, q);
  if((n->l)) findWords(n->l, q);
  if((n->m)) findWords(n->m, q);
  if((n->n)) findWords(n->n, q);
  if((n->o)) findWords(n->o, q);
  if((n->p)) findWords(n->p, q);
  if((n->q)) findWords(n->q, q);
  if((n->r)) findWords(n->r, q);
  if((n->s)) findWords(n->s, q);
  if((n->t)) findWords(n->t, q);
  if((n->u)) findWords(n->u, q);
  if((n->v)) findWords(n->v, q);
  if((n->w)) findWords(n->w, q);
  if((n->x)) findWords(n->x, q);
  if((n->y)) findWords(n->y, q);
  if((n->z)) findWords(n->z, q);
  if((n->space)) findWords(n->space, q);
} // end of findWords
